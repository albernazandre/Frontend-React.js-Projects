...
test('I am your test', async () => {
  ...
  let myFilter = screen.getAllByTestId('filter');
  expect(myFilter).toHaveLength(2);
  const removeOneFilter = screen.getAllByRole('button', {  name: /Exclude/i});
  userEvent.click(removeOneFilter[0]);
  myFilter = screen.getAllByTestId('filter');
  expect(myFilter).toHaveLength(1);
  userEvent.click(removeOneFilter[1]);
  expect(myFilter[0]).not.toBeInTheDocument();
  await act(async () => {
    btnFilter.click();
  })
  myFilter = screen.getByTestId('filter');
  userEvent.click(allFilterRemove);
  expect(myFilter).not.toBeInTheDocument();
});
