...
const { default: aDrinks } = require('../../src/tests/mocks/aDrinks');
const { default: firstLetterMeal } = require('../../src/tests/mocks/firstLetterMeal');
...
Promise.resolve({
  ...
  json: () => {
    ...
    if (url === 'https://www.thecocktaildb.com/api/json/v1/1/search.php?f=a')
      return Promise.resolve(aDrinks)

    if(url === 'https://www.themealdb.com/api/json/v1/1/search.php?f=a')
    return Promise.resolve(firstLetterMeal)
      console.log(url)
