...
// Selecionando a classe da lista de produtos no HTML (cart__products)
...
// funcao que soma os preços dos produtos
const priceAll = () => {
  const allValue = Array.from(productCart.querySelectorAll('.product__price__value')) // Array.from cria um novo array a partir dos valores dos produtos adicionados ao carrinho
    .reduce((acc, curr) => Number(curr.innerHTML) + acc, 0); // reduce vai acumular os valores do array de preços dos produtos do carrinho
  document.querySelector('.total-price').innerHTML = allValue;
};
...
// funcao assincrona que acontece enquanto elementos do quadro do produto são carrregados
export async function addingProduct(ID) {
  const dataOfProduct = await fetchProduct(ID); // espere a funcao que recebe como parmetro o id dar o seu retorno
  const productElement = createCartProductElement(await dataOfProduct); // espere a funcao que cria todos os elementos do carrinho retornar
  productCart.appendChild(productElement);// a classe de listagem de produtos 'cart__products' recebe as informações
  priceAll();
}
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async (event) => { // async faz aguardar o clique para iniciar a funcao
    const identifProduct = getIdFromProduct(event.target.parentElement); // o event é  clique, o target é onde cliquei, e o element é o que eu selecionei
    saveCartID(identifProduct); // salva as info na localstorage
    addingProduct(identifProduct); // adiciona o card no carrinho
  });
